...
**CI-4:** Added structured JSON logging and CloudWatch log shipping. `_JsonFormatter` installed on the root logger in `main.py` emits one JSON object per log record — enables CloudWatch Insights queries. `docker-compose.prod.yml` updated with `awslogs` log driver on all three Python services (`backend`, `celery_worker`, `celery_beat`) via a shared YAML anchor. Requires `CLOUDWATCH_LOG_GROUP` env var and IAM role with `logs:Put*` permissions on the host.
...
- CI-4: All production container logs are now shipped to CloudWatch as structured JSON. Durable across container restarts; queryable via CloudWatch Insights.
