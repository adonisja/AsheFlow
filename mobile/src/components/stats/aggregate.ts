/**
 * PORTED VERBATIM from `frontend/src/components/stats/aggregate.ts`.
 *
 * ADR-271 B put aggregation on the client, which means this logic exists twice.
 * The two copies must stay identical: a change to bucketing, trend rules or
 * date handling lands on BOTH surfaces in the same commit — the rule ADR-269
 * set for the supervised block. Only the type import path differs.
 *
 * Verify with:
 *   diff <(tail -n +N mobile/src/components/stats/aggregate.ts) \
 *        <(tail -n +2 frontend/src/components/stats/aggregate.ts)
 */
/**
 ...
 */
import type { DayStat, YearStat } from './types';
