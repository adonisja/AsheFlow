/**
 * PORTED VERBATIM from `frontend/src/components/notifications/classify.ts`.
 *
 * ADR-275 D4: the same rule decides what a walker sees on a phone in a van and
 * what a dispatcher sees on a desktop. The two copies must stay identical — a
 * change lands on both in the same commit.
 *
 * Verify with:
 *   diff <(tail -n +N mobile/src/components/notifications/classify.ts) \
